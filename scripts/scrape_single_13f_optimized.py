...
- Single-pass XML parsing (10x fewer iterations per holding)

Usage:
    python scrape_single_13f_optimized.py --csv input.csv
    python scrape_single_13f_optimized.py --csv input.csv --workers 12
"""
import argparse
import csv
import os
...
def fetch_holdings_raw(xml_url):
    ...
    OPTIMIZED: Builds a single lookup dict per infoTable element instead of
    iterating through children multiple times per field (~10x fewer iterations).
    """
    try:
        ...
        try:
            for _, elem in etree.iterparse(io.BytesIO(r.content), events=("end",), recover=True):
                ...
                # OPTIMIZATION: Build lookup dict ONCE per infoTable element
                # This replaces 10+ iterations through children with a single pass
                values = {}
                ...
                for child in elem.iter():
                    ...
                    # Handle votingAuthority nested structure inline
                    if ctag == 'votingauthority':
                        for vchild in child:
                            if not isinstance(vchild.tag, str) or not vchild.text:
                                continue
                            vtag = vchild.tag.rsplit('}', 1)[-1].lower()
                            try:
                                val = int(vchild.text.replace(',', ''))
                                if vtag == 'sole':
                                    voting_sole = val
                                elif vtag == 'shared':
                                    voting_shared = val
                                ...
                                pass
                    elif child.text and ctag not in values:  # First occurrence wins
                        values[ctag] = child.text.strip()

                # Extract values using dict lookup O(1) vs O(n) iteration
                val_value = values.get("value")
                val_shares = values.get("sshprnamt")

                try:
                    parsed_value = int(val_value.replace(',', '').replace(' ', '')) if val_value else None
                except:
                    parsed_value = None

                try:
                    parsed_shares = float(val_shares.replace(',', '').replace(' ', '')) if val_shares else None
                except:
                    parsed_shares = None

                holdings.append((
                    idx,
                    values.get("nameofissuer"),
                    values.get("titleofclass"),
                    values.get("cusip"),
                    parsed_value,
                    parsed_shares,
                    values.get("sshprnamttype"),
                    values.get("putcall"),
                    values.get("investmentdiscretion"),
                    values.get("othermanager"),
                    voting_sole,
                    voting_shared,
                    voting_none,
                ))
...
def determine_value_multiplier(holdings_raw, total_value_thousands):
    ...
    Strategies:
    1. AUM Cap Heuristic: If header > $25T, it's in dollars
    2. Share Ratio Heuristic: If value/shares > 2.0, likely dollars
    3. Consistency check with header value
    """
    if not holdings_raw:
        return 1000
...
def apply_value_multiplier(holdings_raw, multiplier):
    ...
    return [
        (h[0], h[1], h[2], h[3],
         h[4] * multiplier if h[4] is not None else None,
...
def commit_batch(cur, conn, pending_inserts):
    """Commit a batch of inserts to the database."""
...
def main():
    ...
    print("=" * 60)
    print("SEC 13F Optimized Batch Scraper v2")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for i, future in enumerate(as_completed(futures), 1):
            ...
            if len(pending_inserts) >= BATCH_COMMIT_SIZE:
                commit_batch(cur, conn, pending_inserts)
